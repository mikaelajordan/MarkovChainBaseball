...
def make_team_transition_matrix(team_code):
    """
    Given a team code, return a {player:player_transition_matrix} dictionary.
    """
...
def team_markov(transition_matrix, iterations=15):
    """
    Given a transition matrix, return the expected number of runs per 9 innings
    """
    x0=np.concatenate([[1], np.zeros(27)])
    x=[x0]
    for i in range(1,iterations):
...
def team_markov_from_raw(all_batting_data, team_code, iterations=15):
    """
    Given all raw batting data and team code, make a team transition matrix and calculate the expected number of runs per game.
    """
    team_atbats = find_team_atbats(team_code, all_batting_data)
    team_t_matrix = make_transition_matrix(team_atbats)
    team_runs = team_markov(team_t_matrix)
    return(team_runs)
